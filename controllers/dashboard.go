...
func (pdc *ProductDashboardController) GetEventReport(c echo.Context) error {
	page := strings.TrimSpace(c.QueryParam("page"))
	limit := strings.TrimSpace(c.QueryParam("limit"))
	sortBy := c.QueryParam("sort_by")
	...
	result, meta, link, err := pdc.productDashboardUsecase.GetEventReport(c, req)
	if err != nil {
		return http_util.HandleErrorResponse(c, http.StatusInternalServerError, msg.FAILED_GET_PRODUCTS)
	}
...
func (pdc *ProductDashboardController) GetChartEvents(c echo.Context) error {
	page := strings.TrimSpace(c.QueryParam("page"))
	limit := strings.TrimSpace(c.QueryParam("limit"))
	sortBy := c.QueryParam("sort_by")
	...
	result, err := pdc.productDashboardUsecase.GetEventChart(c, req)
	if err != nil {
		return http_util.HandleErrorResponse(c, http.StatusInternalServerError, msg.FAILED_GET_PRODUCTS)
	}
